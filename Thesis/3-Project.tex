...
\begin{itemize}
  ...
  \item If $\omega_i^{k_i}\neq 1$ for only one $i$, which we set to be $i=4$, the intersection
    ...
    is located at $(1,0,0,0,x_4)$.% in terms of the coordinates $\mathbb{CP}^4$. 
...
%  \item If $\omega_i^{k_i}\neq 1$ for at least two different values of $i$, we have to deform one of the SLags. 
...
%    Then, the intersection number is the number of zero vectors on the tangent space of the intersection.
%    \todoin{``intersection number equals the number of zeros of a section of the tangent bundle of the intersection locus.``->Is it the minimum number of zero sections. Can't we have a tangent field to the circle that is zero?}
%    \begin{itemize}
%  \item
%    If $\omega_i^{k_i}\neq 1$ for two $i$'s, the intersection is $[1:0:0:x_3:x_4]$, which is homeomorphic to the circle.
%    $I_{\mathbb{RP}^3}=0$
%  \item 
%     If $\omega_i^{k_i}\neq 1$ for three $i$'s, the intersection is $[1:0:0:x_3:x_4]$, which is homeomorphic to the circle.
%$I_{\mathbb{RP}^2}=1$
%  \item 
%    If $\omega_i^{k_i}\neq 1$ for four $i$'s, the intersection is $\mathbb{RP}^3$, which is homeomorphic to $S^3$
%$I_{\mathbb{RP}^3}=0$
    \end{itemize}
\end{itemize}

To compute the total intersection number, we would have to repeat this procedure in all patches
